...
public class I18N {
	...
	public static String nounsHint(Configure configure, Set<String> nounsSet) {
		...
		switch (locale) {
			case "zh":
				return "。严格保证" + nounsStr + "等这些飞行呼号或人名保留原文。严格保证Fox-[1-3], Maverick, Magnum, Rifle, Dirt,Dart, Mud, X-Ray, Alpha, Bravo, Charlie保留原文。坐标保留数字。雷达与战机型号：" + radarStr + "," + aircrafts() + "保留原文。数字翻译例：ninety-nine翻译为99，four-zero-one翻译为401";
			case "ja":
				return "。厳格に保証します" + nounsStr + "これらの飛行コールサインや人名は原文のままです。厳格に保証しますFox-[1-3], Maverick, Magnum, Rifle, Dirt, Dart, Mud, X-Ray, Alpha, Bravo, Charlieは原文のままです。座標は数字を保持します。レーダーモデル：" + radarStr + "は原文のままです。数字の翻訳例：ninety-nineは99に、four-zero-oneは401に翻訳されます";
			case "ko":
				return "。엄격하게 보장합니다" + nounsStr + "이 비행 호출 부호나 인명은 원문 그대로입니다. 엄격하게 보장합니다Fox-[1-3], Maverick, Magnum, Rifle, Dirt, Dart, Mud, X-Ray, Alpha, Bravo, Charlie는 원문 그대로입니다. 좌표는 숫자를 유지합니다. 레이더 모델：" + radarStr + "는 원문 그대로입니다. 숫자 번역 예: ninety-nine는 99로, four-zero-one은 401로 번역됩니다";
			case "es":
				return ". Garantice estrictamente que " + nounsStr + " estos indicativos de vuelo o nombres se mantienen en el texto original. Garantice estrictamente Fox-[1-3], Maverick, Magnum, Rifle, Dirt, Dart, Mud, X-Ray, Alpha, Bravo, Charlie se mantienen en el original. Las coordenadas mantienen los números. Modelo de radar: " + radarStr + " se mantiene en el original. Ejemplo de traducción de números: ninety-nine → 99, four-zero-one → 401";
			case "fr":
				return ". Assurez-vous strictement que " + nounsStr + " ces indicatifs d'appel ou noms propres restent en texte original. Assurez-vous strictement que Fox-[1-3], Maverick, Magnum, Rifle, Dirt, Dart, Mud, X-Ray, Alpha, Bravo, Charlie restent en original. Les coordonnées conservent les nombres. Modèle de radar: " + radarStr + " reste en original. Exemple de traduction des nombres: ninety-nine → 99, four-zero-one → 401";
			case "de":
				return ". Stellen Sie strikt sicher, dass " + nounsStr + " diese Rufzeichen oder Namen im Original bleiben. Stellen Sie strikt sicher, dass Fox-[1-3], Maverick, Magnum, Rifle, Dirt, Dart, Mud, X-Ray, Alpha, Bravo, Charlie im Original bleiben. Koordinaten behalten die Zahlen. Radarmodell: " + radarStr + " bleibt im Original. Zahlenbeispiel: ninety-nine → 99, four-zero-one → 401";
			case "pt":
				return ". Garanta estritamente que " + nounsStr + " estes indicativos de voo ou nomes permaneçam no original. Garanta estritamente Fox-[1-3], Maverick, Magnum, Rifle, Dirt, Dart, Mud, X-Ray, Alpha, Bravo, Charlie permaneçam no original. As coordenadas mantêm os números. Modelo de radar: " + radarStr + " permanece no original. Exemplo de tradução de números: ninety-nine → 99, four-zero-one → 401";
			case "ru":
	...
	private static String aircrafts() {
		return "Flanker, Foxhound, Fulcrum, Frogfoot, Backfire, Bear, Blackjack, Falcon, Hornet, Viper, Hornet, Hog, Warthog, Strike Eagle, Raptor, Rhino, Tomcat, Harrier, Fencer, Blinder, Mainstay, Fullback, Typhoon, Tornado, Hind, Hip, Blackhawk, Chinook";
	}
